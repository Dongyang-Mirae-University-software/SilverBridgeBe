---
name: work-prompt
description: 기능 구현·기존 기능 점검 요청을 받았을 때, 실행 전에 단계(PHASE) 구조의 작업 프롬프트를 설계해 사용자에게 제시하고 "시작해" 승인을 받는다. 생성(구현)·기능 점검·영향 범위 점검·전체 점검 템플릿 4종. 구현을 마치면 점검을 먼저 제안한다(CLAUDE.md §2-6). 사용자가 "~ 구현하자/진행하자", "~ 점검해줘", "설계안대로 만들어줘"라고 할 때 사용.
---
...
1. 요청 성격 판단 → **생성(A)** / **기능 점검(B)** / **영향 범위 점검(C)** / **전체 점검(D)** 템플릿 선택
...
PHASE 4. 점검 제안  (CLAUDE.md §2-6)
  - 이번 변경이 공용 규칙을 건드렸는지 판정 (enum 값 추가·디스패처/NotificationType 정책·
    ConnectionService 인가 메서드·이벤트/리스너 계약)
    → 건드렸으면 템플릿 C(영향 범위), 아니면 템플릿 B(기능) 프롬프트 초안을 제시
  - docs/audit-index.md 에 이 기능 행을 ❌ 로 추가하고, ❌ 가 5건 이상이면 템플릿 D(전체)도 함께 제안
  - 제안만 한다 - 실행은 "시작해" 후
...
산출물
  ...
  ② docs/progress.md 갱신 (PASS/FAIL) + docs/audit-index.md 상태 갱신
  ③ 이슈 발견 시 수정용 커밋 메시지 초안
```

## 템플릿 C - 영향 범위 점검 (공용 규칙 변경 시)

기능 점검은 그 기능의 도메인만 본다. 공용 규칙이 바뀌면 **다른 도메인의 옛 코드**가 조용히 어긋나는데, 이건 기능 점검으로 잡히지 않는다(실제 사례: `RESTRICTED` 추가 후 연결 수락 경로가 보호자 상태를 안 봐 R-1, 역할 변경 후 카메라 고아 M-3).
...
PHASE C. 테스트로 고정할 수 있는가  [스킬: test-quality]
  - enum ↔ DB CHECK 동기화처럼 기계적으로 막을 수 있는 건 *CheckSyncTest 형태로 제안

보고 형식 / 규칙 / 산출물은 템플릿 B와 같다 (문서명: audit-impact-<변경>.md)
```

## 템플릿 D - 전체 점검 (배포·발표 전 / 미점검 5건 이상)

개별 API의 정확성은 기능 점검이 이미 봤다. 전체 점검은 **기능 사이**와 **시간이 지나며 생긴 어긋남**을 본다. 형식 기준은 `docs/(2026-09-10) audit-regression-pre-230.md`·`(2026-09-11) audit-technical-cross-cutting.md`.

```
전체 점검 (기준 커밋: ..., 직전 전체 점검: ...).

PHASE -1. 사전 환경 확인
  - 오늘 날짜 / docs/audit-index.md 의 ❌·⚠️ 목록 / 빌드·전체 테스트 통과
...
PHASE B. 일관성  [스킬: api-contract-review]
  - 에러 코드·상태코드 형태(<도메인>_NOT_AUTHORIZED 403 + [IDOR-ATTEMPT]) / 응답 포맷 / 필터 enum 규칙
...
점검에서 찾은 이슈 중 **기계적으로 재발을 막을 수 있는 것**(enum ↔ CHECK 동기화, 인가 누락, 문구 고정 등)은 수정 제안에 테스트를 포함한다(`UserStatusCheckSyncTest`·`AdminAuditActionCheckSyncTest`가 예). 그래야 다음 전체 점검에서 같은 항목을 다시 보지 않는다.
