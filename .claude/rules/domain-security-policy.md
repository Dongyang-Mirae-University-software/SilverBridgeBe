...
## SOS 이력·ACK — ACTIVE 연결이 유일한 열람 근거 (2026-07-30)

- **경위**: 보호자용 SOS 이력 조회 + 처리 결과(ACK) 추가(`sos_event` 컬럼 확장, V33). SOS는 피보호자의 위치·건강 위기 시점이 드러나는 민감 이력이라 "누가 볼 수 있는지"를 코드 밖에 고정해 둔다.
- **불변 규칙 ①(열람 범위)**: 보호자는 **요청 시점에 ACTIVE 연결인 피보호자**의 이력만 조회·처리할 수 있다. 연결이 해제·거절되면 **과거 이력도 즉시 비공개**가 된다(연결 종료 후 개인정보 잔존 방지). 인가 목록은 `ConnectionService.getActiveWardIds()`·`isActiveConnection()`만 사용한다 — `getMyWards()`는 **PENDING이 섞여 있어 인가 목록으로 쓰면 수락 전 피보호자의 이력이 노출된다**.
  - 피보호자 탈퇴로 `ward_id`가 NULL이 된 익명 이력은 처리 주체가 없으므로 **403으로 막는다**(익명 보존은 감사 목적이며 열람 대상이 아니다).
  - 연결 없는 대상 접근은 404 위장 대신 **403 `SOS_NOT_AUTHORIZED` + `[IDOR-ATTEMPT]` WARN**(2026-07-14 정책과 동일 형태).
- **불변 규칙 ②(ACK는 알림에 개입 금지)**: ACK는 **사후 기록**일 뿐이다. 처리 완료된 SOS라도 이후 SOS 알림 정책은 바뀌지 않으며, `SosNotificationListener`·`NotificationDispatcher`가 `ack_status`를 읽게 만들지 말 것("이미 확인했으니 알림 생략" 같은 최적화 금지 — 필수 알림 보장에 구멍). 2026-07-23 규칙([SOS 동작 설정])의 연장이다.
- **ACK 알림 = WebSocket만**: 수신자는 ACTIVE 보호자 전원 + 피보호자 본인. FCM·SMS·알림톡은 보내지 않는다(상황 종료 후 상태 갱신이라 소음). 새 채널을 붙이자는 요구가 오면 이 판단을 먼저 확인할 것.
- **기록 단위**: 이력 1행 = ACK 1개(마지막 처리로 덮어쓰기, 재ACK 허용). 보호자별 개별 ACK를 원하면 별도 테이블이 필요하며, 그때도 위 열람 규칙은 동일하게 적용한다.
...
- **불변 규칙 ⑧(집계 상한 = 판정 시각)**: 요약은 **판정 시각(기본 21:00 KST)까지 복용 시각이 지난 약만** 센다. 취침 전 22:00 약을 포함하면 "아직 먹을 때가 아닌 약"이 매일 미복용으로 잡혀 거짓 알림이 나간다. 그 약이 요약에서 빠지는 것은 **의도된 동작**이며, 포함하려고 판정 시각을 자정 근처로 늦추지 말 것(보호자가 대응할 수 없는 시간이다).
