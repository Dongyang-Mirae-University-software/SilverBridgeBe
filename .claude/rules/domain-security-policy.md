...
- **관리자 지표 (PR ③ 구현 완료 2026-09-02)**: 오탐률은 **응답률과 함께** 표시한다. 재촉해도 응답률은 100%가 되지 않아, 오탐 건수만 띄우면 분모가 거짓이 된다. SOS 처리결과(ACK)가 전건 NULL인 채 "무응답 100%"로 읽힐 뻔한 함정과 같다. `GET /api/admin/dashboard/safety`의 `todayAnomaly.review`가 pending·real·falseAlarm·conflicted **네 값을 모두** 내리는 이유가 이것이다 - 응답 수는 `total - pending`이다. **오탐 건수만 단독으로 노출하는 응답을 만들지 말 것.**
  ...
  - **대시보드 조회는 감사 로그(`admin_audit_log`)에 남기지 않는다**: 집계 숫자만 반환해 개인 식별 정보가 없고, 30초 폴링 화면이라 기록하면 공지 수정 같은 실제 조작 이력이 묻힌다. 다만 **개인 이력을 변경하는 관리자 API(판정 정정·회원 수정·강제 연결 등)는 반드시 남긴다** - 이 예외를 확대 적용하지 말 것. 조회(목록·상세·이상감지 로그)는 남기지 않는다 - 페이징·폴링 조회까지 기록하면 실제 조작 이력이 묻힌다(2026-09-10 점검 M-4에서 문구 정정. 옛 문구 "열람하는"은 구현과 달랐다).
...
- **불변 규칙 ⑦(정지 계정에는 알림을 보내지 않는다, 2026-09-09 추가)**: 이용 제한·탈퇴 진행 계정이 **수신자**면 어떤 채널로도 발송하지 않는다 - **강제 FCM도 예외가 아니다**(`NotificationDispatcher.resolveIfAllowed`). 정지는 탈취 의심 시의 임시 조치라, 알림을 계속 보내면 그 계정을 쥔 사람에게 피보호자의 SOS 발생·화재 감지 같은 생활 상황을 실시간으로 통보하게 된다. "필수 알림이니 예외" 논리로 되돌리지 말 것.
  ...
  - **수용한 한계 - 열린 WebSocket 세션 (2026-09-10 점검 M-5)**: 무효화 키는 `JwtHandshakeInterceptor`가 **핸드셰이크 때만** 검사한다. 정지 시점에 이미 열려 있던 STOMP 세션은 서버가 끊지 않아 `anomaly-detected`·`medication-taken`·`connection-*` 실시간 이벤트가 그 브라우저에 계속 도착한다. HTTP는 즉시 401이라 프론트가 로그아웃 처리하며 소켓을 닫는 것이 일반적이지만 백엔드가 보장하지는 않는다. 서버 측 강제 종료는 핸드셰이크가 Principal을 세우지 않아(`SimpUserRegistry`로 세션을 못 찾음) 현재 구조로는 어렵다 - 필요해지면 핸드셰이크에 Principal을 붙이는 작업이 선행되어야 한다. 2026-09-09 기록의 "정지되면 세션 없음"은 새 로그인에만 참이었다.
  - **"유일한 보호자 경고"는 만들지 않기로 했다 (2026-09-09 결정)**: 정지 대상을 **보안 이상·법적 문제 계정으로 한정**했으므로, 유일한 보호자여도 정지 결정은 바뀌지 않는다. 경고를 봐도 관리자가 할 일이 같아 화면만 복잡해진다. **위험 자체가 사라진 것은 아니다** - 정지를 실제로 쓰기 시작해 이 경우가 생기면 그때 다시 판단한다. 미구현 항목이 아니라 **의도적으로 만들지 않은 것**이므로, "빠졌다"고 보고 다시 만들지 말 것.
    - ⚠️ **정지가 만드는 사각지대는 대시보드에도 안 잡힌다**: 강제 탈퇴·역할 변경은 연결이 사라져 `wardsWithoutGuardian`에 잡히지만, **정지는 연결이 그대로 남아** 지표상 멀쩡해 보인다. 지표를 근거로 "사각지대 없음"을 판단하지 말 것.
