...
- **현황(2026-07-20)**: 이상감지 **보호자용** 템플릿 `KA01TP260715015020754dXeU0ww3my9` — 1차 **반려**(수신 대상·발송 사유 불명확) → 문구 전면 재작성 후 **재검수 요청**(동일 템플릿 수정이라 ID 유지 추정, 승인 시 실제 ID 확인 필요). 카테고리 `서비스이용 > 이용안내/공지(004001)`, 기본형, 대체발송 OFF. 발신 프로필 `KA01PF240930145539248iUN6bVyplGB`. 승인 전까지 `ALIMTALK_ENABLED=false`(채널 스킵). `pfId`·`templateId`·Solapi 키는 `.env.dev` 주입(평문 커밋 금지).
