...
- **복약 알림**(medication, 2026-08-04, **1차 = 등록·체크·조회**): 보호자가 약을 등록(`POST /api/guardian/ward/{wardId}/medication`)하고 **피보호자만 복용을 체크**(`POST /api/ward/medication/{id}/intake`)한다 — 이 역할 분리가 기능의 목적이라 **보호자용 체크 API·피보호자용 등록 API를 만들지 말 것**. 인가는 ACTIVE 연결만(`getActiveWardIds`·`isActiveConnection`; `getMyWards`는 PENDING 혼입이라 금지), 위반 시 403 + `[IDOR-ATTEMPT]`. 날짜는 항상 **KST**(`MedicationClock`), 체크는 `UNIQUE(medication_id, dose_date)`로 멱등, 약 삭제는 soft delete(복용 이력 보존). 체크 알림은 **WebSocket만**(`medication-taken`). 등록 보호자 탈퇴 시 그 약도 삭제하고 **남은 보호자에게만** 중지 안내(`MEDICATION_STOPPED`). 복용 시각 알림 발송(스케줄러)은 **2차** — `medication_setting.alarm_enabled`는 지금 보관만 한다.
