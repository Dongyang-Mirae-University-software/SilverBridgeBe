...
- **이상감지**(anomaly, 2026-07-14, **1·2단계 완료**): AI 서버는 웹훅이 없어 백엔드가 **AI WS를 클라이언트로 구독**(`AiLiveStreamSubscriber`, AI 무변경). 등록된 `camera.session_id`만 subscribe → 판정 → 이력 쿨다운 → `anomaly_event` 적재 → `AnomalyDetectedEvent` → AFTER_COMMIT 리스너가 **ACTIVE 보호자 전원 + 피보호자 본인**에게 발송(WS `anomaly-detected` + FCM 고정 / SMS·알림톡은 설정대로). 쿨다운은 **이력 1분 / 알림 보호자 5분·본인 3분**(서로 별개 — 이력은 촘촘히, 사람에겐 성기게).
  ...
  - **판정·재촉**(2026-09-01, V44 스키마 / V45 재촉): 연속 감지를 **상황(incident)**으로 묶어(같은 `(ward, session, type)`이고 직전 감지로부터 **10분 이내**면 승계, KST 자정은 안 넘김) 판정·통계의 단위로 삼는다. 판정은 **보호자만**(1인 1표·번복 가능, `REAL`/`FALSE_ALARM`) — 피보호자·관리자용 판정 API를 만들지 말 것. **응답자 다수결**이고 **동수만 `CONFLICTED`**(2026-09-21 변경, V52) - 동수는 보호자들이 다시 응답해 합의하며, 응답한 다른 보호자에게 재확인 안내(`ANOMALY_REVIEW_CONFLICTED`, FCM, 보호자당 1회)가 간다. 판정은 **이미 나간 알림을 되돌리지 않는다**. 인가는 ACTIVE 연결만, 위반 시 403 + `[IDOR-ATTEMPT]`.
    - **미응답 재촉**: 상황이 닫힌 뒤 1시간 → 건별 FCM 1회 → 이후 **하루 1회 요약**(20:00 KST), 마감 3일, 야간 22:00~08:00 억제(**화재 알림 본체는 억제 안 함**), **채널 FCM만**, 보호자별 끄기 필수, 선점 후 발송. 킬 스위치 `anomaly.review-reminder.enabled`. 한 명이 답하면 **나머지 재촉만 중단**하고 응답 API는 열어둔다. 상세·금지사항은 `.claude/rules/domain-security-policy.md`.
    - **관리자 대시보드**(2026-09-02, 마이그레이션 없음): `GET /api/admin/dashboard/safety`·`/operation` 2종(탭별 분리). **모르는 값을 0으로 채우지 않는다** — AI WS가 끊기면 `streamingCameras`·`disconnectedCameras`는 `null`(카메라에 스트리밍 상태 컬럼이 없어 AI 구독 집합에서 파생하므로), 미답변 문의 0건이면 `longestWaitingHours`도 `null`. **오탐률은 응답률과 함께** 쓰도록 `review` 4값을 모두 내린다(응답 수 = `total - pending`). `byType`은 집계된 유형만(0건 유형은 항목 없음). 날짜는 KST, 서버 캐시 없음, 조회는 감사로그 미기록.
    - **관리자 로그**(2026-09-02 / 정정 폐지·v2 2026-09-21): `GET /api/admin/anomaly`(상태·피보호자·**기간(KST, 주=월요일)·유형·이름/장소 검색** 필터, 보호자 응답 내역·감지 지속 포함) + `GET /api/admin/anomaly/summary`(유형 탭 건수는 유형 필터 무시 / 응답률 / 판정 4값 / **AI 신뢰도 = 판정 난 상황(위험+오탐)의 confidence 평균**(`aiConfidence{average, basis}`, 2026-09-22 비율에서 교체 - 카드 하위 칸은 FE가 `review.real/basis`로 계산), 분모 0이면 null. AI confidence가 0~1 밖이면 파서가 잘라 넣고 `[ANOMALY-CONFIDENCE-OUT-OF-RANGE]` WARN(신호는 버리지 않음)) **조회 전용**. 관리자 정정 `PATCH .../review`는 **폐지** - 관리자는 판정하지 않는다(쓰기 매핑 없음을 테스트로 고정). `resolved_*` 컬럼·`ANOMALY_REVIEW_RESOLVE` enum·CHECK는 과거 기록 보존용으로 남겼다. ⚠️ `AdminAuditAction`에 값을 더할 때는 **CHECK 재정의 마이그레이션 필수**(`AdminAuditActionCheckSyncTest`).
  - **카카오 알림톡 = 보호자 전용**(템플릿 2026-07-27 승인). 본인 수신분은 `ANOMALY_DETECTED_SELF`로 dispatch되고 이 타입엔 템플릿 매핑이 없어 **알림톡만 스킵**(FCM·WS·SMS는 그대로). **두 서버 모두 `ALIMTALK_ENABLED=true`로 실제 발송 중**(2026-07-31 확인, 로컬 기본값은 `false`) — 상세는 `.claude/rules/domain-security-policy.md`. WS 연결 실패·`AI_API_KEY` 미설정은 **기동을 막지 않음**(구독만 비활성).
- **복약 알림**(medication, 2026-08-04 1차 / 발송 2026-08-05, **1·2차 완료**): 보호자가 약을 등록(`POST /api/guardian/ward/{wardId}/medication`)하고 **피보호자만 복용을 체크**(`POST /api/ward/medication/{id}/intake`)한다 — 이 역할 분리가 기능의 목적이라 **보호자용 체크 API·피보호자용 등록 API를 만들지 말 것**. 인가는 ACTIVE 연결만(`getActiveWardIds`·`isActiveConnection`; `getMyWards`는 PENDING 혼입이라 금지), 위반 시 403 + `[IDOR-ATTEMPT]`. 날짜는 항상 **KST**(`MedicationClock`), 체크는 `UNIQUE(medication_id, dose_date)`로 멱등, 약 삭제는 soft delete(복용 이력 보존). 체크 알림은 **WebSocket만**(`medication-taken`). 등록 보호자 탈퇴 시 그 약도 삭제하고 **남은 보호자에게만** 중지 안내(`MEDICATION_STOPPED`).
  - **수정**(`PATCH /api/guardian/medication/{id}`, 2026-08-05): 부분 수정(null=미변경). `timeSlot`만 바꾸고 `doseTime`을 생략하면 **새 슬롯 기본 시각으로 갱신**("저녁 08:00" 방지), 메모는 **빈 문자열로 삭제**. **복용 시각이 바뀌면 당일 발송 기록(attempt 1·2)을 삭제**해 새 시각으로 다시 알림이 나가게 하되 **복용 체크는 유지**한다. `wardId`·`createdBy`는 불변.
  - **미복용 요약**(3차, V37 / 시각 선택 V40 / 피보호자별 V41): **보호자가 그 피보호자에 대해 지정한 시각**(미설정 시 21:00 KST)에 **ACTIVE 보호자 중 수신 설정 ON**인 사람에게 `MEDICATION_MISSED`(FCM·문자) 발송. **지정 시각 = 집계 상한** — 그 시각까지 복용 시각이 지난 약만 집계한다(21:00이면 취침 전 22:00 약이 빠짐 — 의도된 동작이라 시각을 그 피보호자의 마지막 복약 시각에 맞춰 잡는 것이 이 설정의 요점). 하루 1건(`UNIQUE(guardian_id, ward_id, dose_date)`), 미체크 0건이면 미발송. ⚠️ 문구는 **"체크되지 않았습니다"** — "안 드셨습니다"로 쓰지 말 것(체크 누락과 실제 미복용을 구분할 수 없다). 설정 API `GET·PUT /api/guardian/ward/{wardId}/medication-alert-setting`(**(보호자, 피보호자) 축** — 같은 피보호자를 보는 보호자끼리 시각이 다를 수 있다. 두 필드 모두 선택=미변경, 위반 시 403+`[IDOR-ATTEMPT]`). 카드 조회(`GET /api/guardian/medication`) 응답에 설정 4종이 동봉된다. 킬 스위치 `medication.reminder.missed-alert.enabled`는 **2차와 독립**.
  - **발송**(2차, V36): 스케줄러 1분 주기 → 복용 시각 도래 + 미체크 + 알림 ON이면 **피보호자 본인에게** `MEDICATION_REMINDER`(SETTINGS_ONLY) 발송. **채널은 FCM·문자뿐** — 알림톡은 `templates` 매핑을 두지 않아 스킵(다발성 메시지라 별도 템플릿 승인 필요), WS도 안 보냄. 유예 **30분**(자정은 안 넘김), **재알림은 사용자 선택**(`remind_again_enabled`, 기본 ON, 15분 뒤 1회, 마감 60분). 중복 발송은 **"선점 후 발송"**(`medication_reminder_log` + `UNIQUE(medication_id, dose_date, attempt)`)으로 막는다 — 순서를 뒤집지 말 것. 킬 스위치 `medication.reminder.enabled`.
- **알림 채널 추상화**(notification, 2026-05-31 / 정책 확장·알림톡 2026-07-15): 이벤트 → `NotificationDispatcher` → `NotificationType.Policy`에 따라 채널 결정. **`SETTINGS_ONLY`**(연결·문의 = 사용자 설정대로) / **`FORCED_PUSH_WITH_SMS_FALLBACK`**(`WARD_SOS` = FCM 강제 + 미전달 시 SMS 폴백) / **`FORCED_PUSH_PLUS_SETTINGS`**(`ANOMALY_DETECTED` = FCM 고정 + 나머지는 설정대로, **SMS 폴백 없음** — 문자는 사용자 선택이라 폴백이 그 선택을 뒤집음). 채널=`NotificationChannel` 전략(FCM·SMS·**KAKAO_ALIMTALK** 구현, EMAIL은 enum만). **종류별 허용 채널**(2026-09-01): `NotificationType`이 설정 기반 발송에 쓸 채널을 선언하면 디스패처가 *사용자 활성 채널 ∩ 허용 채널*로 좁힌다(기본=전 채널이라 기존 종류 무변경). ⚠️ **강제 FCM은 이 값으로 줄어들지 않는다**. **새 채널 = 구현체 빈 추가**만(디스패처 자동 수집). WebSocket은 추상화 밖(항상 발송). **SMS 인증번호는 디스패처 미경유**(설정 무시). 기본값 FCM ON. 설정 API `/api/user/me/notification-settings`(GET·PUT).
  - **발송 이력**(2026-09-22, V54): 채널 `send()`는 `ChannelResult(status, reason)`를 돌려주고 디스패처가 **수신자 1명당 1행**을 `notification_log`에 남긴다(발송 후, `REQUIRES_NEW`, 실패는 WARN만 - 발송 우선). 결과 `DELIVERED`·`SMS_FALLBACK`·`FAILED`·`NOT_SENT`(**실패 아님**). 사유는 **고정 코드만**(예외 원문 금지). 관련 피보호자는 4인자 `dispatch(userId, wardId, …)`(표시 전용 - `data["wardId"]`로 대신하지 말 것). 조회 `GET /api/admin/notification`·`/summary`(ADMIN, 조회 전용). 보관 90일. 상세 `docs/(2026-09-22) feature-admin-notification-history.md`.
...
6. **점검은 AI가 시점을 보고 먼저 제안** - 아래에 해당하면 작업을 마친 뒤 점검 프롬프트를 **제안**한다(실행은 "시작해" 후). 절차는 `work-prompt` 템플릿 B·C·D.
   - **기능 점검**(B): 구현 PHASE 3까지 끝나면 곧바로 - 그 기능 + 건드린 공용 코드의 사용처까지.
   - **영향 범위 점검**(C): 공용 규칙을 바꿨을 때 - `Status`·`AdminAuditAction` 등 enum 값 추가, `NotificationDispatcher`·`NotificationType` 정책, `ConnectionService` 인가 메서드, 이벤트·리스너 계약. **전 도메인 사용처**를 훑는다(R-1이 이 틈으로 샜다).
   - **전체 점검**(D): 배포·발표 전, 큰 기능 묶음 완료 시, 또는 `docs/audit-index.md`에 ❌ 미점검 기능이 **5건 이상** 쌓였을 때.
...
- **관리자 강제 연결**(2026-09-10): 피보호자 수락(=동의) 없이 만드는 관계라 **양쪽 알림 + 감사 로그 필수**. 문구는 전용 종류로(`CONNECTION_FORCED`·`DisconnectedBy.ADMIN`) — 기존 "수락했습니다"·"보호자가 해제했습니다"를 재사용하면 거짓이 된다.
