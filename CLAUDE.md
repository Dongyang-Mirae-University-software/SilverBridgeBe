...
- **이상감지**(anomaly, 2026-07-14, **1·2단계 완료**): AI 서버는 웹훅이 없어 백엔드가 **AI WS를 클라이언트로 구독**(`AiLiveStreamSubscriber`, AI 무변경). 등록된 `camera.session_id`만 subscribe → 판정 → 이력 쿨다운 → `anomaly_event` 적재 → `AnomalyDetectedEvent` → AFTER_COMMIT 리스너가 **ACTIVE 보호자 전원 + 피보호자 본인**에게 발송(WS `anomaly-detected` + FCM 고정 / SMS·알림톡은 설정대로). 쿨다운은 **이력 1분 / 알림 보호자 5분·본인 3분**(서로 별개 — 이력은 촘촘히, 사람에겐 성기게).
  ...
  - **판정·재촉**(2026-09-01, V44 스키마 / V45 재촉): 연속 감지를 **상황(incident)**으로 묶어(같은 `(ward, session, type)`이고 직전 감지로부터 **10분 이내**면 승계, KST 자정은 안 넘김) 판정·통계의 단위로 삼는다. 판정은 **보호자만**(1인 1표·번복 가능, `REAL`/`FALSE_ALARM`) — 피보호자·관리자용 1차 판정 API를 만들지 말 것. 갈리면 `CONFLICTED`이고 **다수결로 정하지 않는다**(불일치 자체가 관리자가 볼 정보). 관리자 정정이 최종. 판정은 **이미 나간 알림을 되돌리지 않는다**. 인가는 ACTIVE 연결만, 위반 시 403 + `[IDOR-ATTEMPT]`.
    ...
    - **관리자 로그·정정**(2026-09-02, V46): `GET /api/admin/anomaly`(상태·피보호자 필터, 보호자 응답 내역 포함) + `PATCH /api/admin/anomaly/{incidentId}/review`. **관리자는 1차 판정을 하지 않는다** — 2차 정정만. 정정은 **상태만 바꾸고 보호자 응답 원본은 남긴다**. `REAL`·`FALSE_ALARM`으로만 확정(되돌리기 400), **재정정은 허용**(관리자도 오조작하므로), 매 정정 **감사 로그 필수**(대시보드가 안 남기는 것과 대비 — 개인 이력을 여는 경로라서). 연결 여부로 좁히지 않는다. 정정 알림 없음. ⚠️ V46은 정정 컬럼(V44가 이미 만듦)이 아니라 **`admin_audit_log`의 CHECK 재정의** 때문이다 — enum만 늘리면 insert가 실패해 본 작업까지 롤백된다(C-S3-1 재발, `AdminAuditActionCheckSyncTest`가 막는다).
