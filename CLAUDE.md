...
### 설계안·기능 구현 요청 = 단계 게이트 (승인 없이 코드 작성 시작 금지)

기능/설계안 기반 작업은 아래 순서를 지킨다. **PHASE 1 승인 전에는 코드를 쓰지 않는다.**

| 단계 | 할 일 |
|---|---|
| **PHASE 0 — 전제 검증** | 설계 문서·지시 프롬프트가 "있다/없다"고 가정한 것을 **코드에서 직접 확인**한다: 대상 클래스·메서드·enum 값의 실재 여부, **Flyway 최신 버전 번호**, 재사용 대상 패턴, 빌드·git 상태. **문서를 사실로 믿지 않는다.** |
| **PHASE 1 — 계획 보고** | 전제와 실제의 **차이(drift)를 먼저 보고**하고, 그 위에서 작업 범위·변경 파일·기존 동작 보존 여부를 제시 → **승인 대기** |
| **PHASE 2 — 구현** | 승인된 범위만. 산출물: 코드+테스트 · `docs/(YYYY-MM-DD) 파일내용.md` · `docs/progress.md` 갱신 · **커밋 메시지 초안**(자동 커밋 금지, §3) |

⚠️ 설계 문서는 **작성 시점의 스냅샷**이라 리포보다 뒤처져 있을 수 있다(이미 머지된 항목을 "미구현"으로 적어둔 채). 문서 §작업목록을 그대로 구현하면 중복 생성·되돌림이 난다 — PHASE 0이 이걸 막는 관문이다.
