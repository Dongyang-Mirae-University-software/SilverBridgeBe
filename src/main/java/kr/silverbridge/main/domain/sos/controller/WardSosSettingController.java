...
public class WardSosSettingController {
    ...
    @Operation(
            ...
            description = """
                    ...
                    [값의 의미 — "119를 어떻게 연결·안내할지"]
                    - CALL_119              : 119 즉시 연결 (버튼을 누르면 곧바로 119 연결로 이동)
                    - CALL_119_AND_NOTIFY   : 119 연결 + 보호자 알림 안내 (기본값)
                    - NOTIFY_GUARDIAN_FIRST : 보호자에게 먼저 알린 뒤 119 연결 방법 안내

                    [⚠️ 중요]
                    이 설정으로 보호자 알림을 끌 수 없습니다. SOS는 생명과 직결된 필수 알림이라
                    어떤 값이든 보호자 알림은 항상 발송됩니다. CALL_119도 "보호자 알림 없이"라는 뜻이 아닙니다.
                    세 값의 차이는 119 연결 시점·안내 방식뿐입니다.

                    [기본값] 설정한 적 없으면 CALL_119_AND_NOTIFY로 응답합니다.

                    [범위 밖 — 프론트 처리] 실제 119 전화 연결은 프론트의 tel:119 링크가 담당합니다.
