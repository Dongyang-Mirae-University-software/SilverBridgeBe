...
/**
 * 보호자가 특정 피보호자에 대해 받을 미복용 요약 설정. 저장된 행이 없으면 기본값(ON · 21:00)이 담긴다.
 *
 * <p>시각은 언제나 <b>실효값</b>이다 - 미설정 여부를 노출하지 않고 "지금 몇 시에 받는지"만 답한다.</p>
 */
@Schema(description = "보호자 미복용 요약 수신 설정 (피보호자별)")
public record GuardianMedicationAlertSettingResponse(

        @Schema(description = "대상 피보호자 ID", example = "A1B2C3")
        String wardId,
    ...
    public static GuardianMedicationAlertSettingResponse of(String wardId, GuardianMissedAlertSetting setting) {
        return new GuardianMedicationAlertSettingResponse(wardId, setting.enabled(), setting.alertTime());
    }
}
