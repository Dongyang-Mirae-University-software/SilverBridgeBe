        ...
        return new OpenAPI()
                ...
                .tags(List.of(
                        ...
                        new Tag().name("관리자 - 이상감지")
                                .description("이상감지 기록 목록(보호자 응답 내역 포함) 조회 전용.\n"
                                        + "관리자는 판정하지 않습니다 - 판정은 응답한 보호자의 다수결이고, 동수(CONFLICTED)는 보호자들이 다시 응답해 풉니다.\n"
                                        + "※ ADMIN 권한 계정만 호출 가능."),
                        ...
                        new Tag().name("관리자 - 회원관리")
                                .description("회원 목록(검색·역할/계정/연결 상태 필터·페이징)·탭 건수·상세 조회, 정보 수정, 강제 탈퇴.\n"
                                        ...
                                        + "이용 제한 계정은 로그인·토큰 재발급이 막히고 어떤 알림도 받지 않습니다(강제 푸시 포함).\n"
                                        + "※ 피보호자는 이용 제한할 수 없습니다(400) — 로그인이 막히면 SOS를 보낼 수 없게 됩니다.\n"
