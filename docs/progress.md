...
## [2026-07-27] 알림톡 이상감지 템플릿 승인 + 본인 수신분 알림톡 차단

- **3차 승인**(`KA01TP260715015020754dXeU0ww3my9`, 2026-07-27). 1차 반려=수신자 액션 불명확, 2차 반려=**다발성 수신 동의 고지 누락** → 검수자 제시 예시 문장을 **고정 문구**로 넣고 비어 있던 검수자 참고 의견을 채워 통과. 제출 원문·차수별 diff는 `docs/(2026-07-23) alimtalk-template-review-history.md`(승인 후 문구 수정은 재검수 대상이라 원문 보존 필수).
- **결정: 이상감지 알림톡은 보호자 전용** — 승인 문구가 보호자용("회원님께서 보호자로 등록하시고 직접 신청하신… 대상: OOO님")이라 피보호자 본인에게 보내면 사실과 어긋나 채널 제재 사유가 된다. **본인용 템플릿은 등록하지 않는다.**
- **구현**: `NotificationType.ANOMALY_DETECTED_SELF` 추가(정책은 보호자분과 동일) + `application.yaml`에 이 타입의 템플릿 매핑을 **두지 않음** → 알림톡만 스킵. 본인은 FCM·WebSocket·SMS를 그대로 받아 **대피 안내(D-1)는 유지**된다.
- **채널 인터페이스 변경**: `NotificationChannel.send(type, recipient, content)` — 알림톡이 승인 템플릿을 고르는 근거를 `data["type"]`(FE 계약 문자열) 스니핑에서 **dispatch된 알림 종류**로 옮겼다. `data["type"]`은 `ANOMALY_DETECTED` 그대로라 FE 계약 무변경.
- **거부안**: 디스패처에 "제외 채널" 파라미터 추가 — 호출자가 FCM 강제 발송까지 끌 수 있게 되어 필수 알림 보장에 구멍.
- **테스트**: `./gradlew build` 전체 통과(알림/이상감지 관련 63건 실패 0, 신규 4건 — 본인 타입 분기·템플릿 선택 근거·채널 종류 전달·정책 가드 확장). 마이그레이션 없음.
- **남은 것**: 각 환경 `.env.dev`에 `ALIMTALK_ENABLED=true` 주입(=발송 시작). `pfId`·`templateId`는 이미 승인 값과 동일.

## [2026-07-28] 이상감지 알림 빈도 조정 (환경변수)

- **이력 쿨다운 5분 → 1분**(`ANOMALY_COOLDOWN_MINUTES`). 화재가 지속되는 구간이 1분 단위로 `anomaly_event`에 남아 사후 타임라인 분석이 가능해진다. AI는 매 프레임 broadcast하므로 쿨다운이 없으면 동일 행이 폭주한다 — 1분은 그 하한.
- **보호자 알림 5분 유지**(`ANOMALY_NOTIFY_COOLDOWN_MINUTES`, 명시). 이력이 잦아져도 보호자가 받는 FCM·알림톡·SMS 건수는 그대로다. 알림톡 건수가 늘면 과금뿐 아니라 **카카오 신고·발신 프로필 차단 리스크**가 커지므로 의도적으로 분리했다.
- **본인 알림 1분**(`ANOMALY_NOTIFY_SELF_COOLDOWN_MINUTES`, 명시). 종전엔 상류의 이력 쿨다운(5분)에 막혀 "본인은 더 짧게"(설계 D-1)가 **실효가 없었다**. 이력 쿨다운을 낮추면서 비로소 의도대로 동작한다.
- **반영 위치 = 각 서버 `.env.dev`** (gosky·vkcs-linux 모두, 컨테이너 재생성 후 확인 완료). 코드 기본값(`AnomalyProperties` 5/5/1)은 그대로라 **로컬 개발 환경은 종전 동작**이다.
- 마이그레이션·코드 변경 없음. 기존 `.env.dev`는 각 서버에 `.env.dev.bak-freq-20260728`로 백업.

### 후속(같은 날): 기본값 일원화 + 본인 간격 3분
...
## [2026-08-05] 미복용 시 보호자 알림 (3차)

- **발단**: 2차로 피보호자 고리는 닫혔지만(복용 시각 알림 + 재알림), 끝내 체크하지 않으면 그 뒤 아무 일도 없었다 — 보호자는 앱을 열어야만 알 수 있었다.
- **PHASE 0 관측 결과 = 데이터 0**: 양 서버 모두 등록된 약·복용 체크·알림 발송 전부 0건(1차는 하루 전, 2차는 당일 배포 + **FE 복약 화면이 9월 시작**). "체크율을 보고 건별/요약을 정한다"는 계획이 성립하지 않아, 추천안(하루 요약·21:00)을 채택하되 **판정 시각·마감·ON/OFF를 전부 환경변수화**해 실사용 후 코드 수정 없이 조정 가능하게 했다.
- **핵심 원칙 두 가지** — ① **"체크 안 함"≠"안 드심"**: 문구는 "아직 체크되지 않았습니다"이며 `doesNotContain("안 드셨")`로 테스트 고정. 보호자에게 사실 아닌 단정을 통보하면 불필요한 걱정·전화를 만든다. ② **알림 피로가 필수 알림을 죽인다**: 보호자가 앱 알림을 통째로 끄면 SOS·이상감지까지 함께 죽으므로, 건별이 아닌 **하루 1건 요약** + **이 알림만 끄는 설정**을 함께 넣었다.
- **판정**: 판정 시각(기본 21:00 KST)**까지 복용 시각이 지난 약만** 집계한다. 취침 전 22:00 약은 아직 먹을 때가 아니라 제외 — 포함하면 매일 거짓 알림이 나간다. 대신 그 약은 그날 요약에서 아예 빠지며, 문구를 "오늘 **저녁까지 예정된** N건 중"으로 적어 해소한다. 발송 창 `[21:00, +120분]`(자정 넘기면 그날 끝에서 절단), 미체크 0건이면 미발송.
- **수신자**: ACTIVE 보호자 중 설정 ON인 사람만. 피보호자 본인 제외(2차가 담당).
- **중복 방지**: `medication_missed_alert_log` `UNIQUE(guardian_id, ward_id, dose_date)` — 하루 한 번. 축이 **약 단위가 아니라 (보호자, 피보호자, 날짜)** 라 2차의 `medication_reminder_log`와 테이블을 분리했다(요약이라 약 3건 미체크여도 알림 1건). **선점 후 발송**은 2차와 동일.
- **채널**: `MEDICATION_MISSED(SETTINGS_ONLY)` — FCM·문자만, 알림톡 매핑 없음(불변 규칙 유지).
- **신규 API**: `GET·PUT /api/guardian/medication-alert-setting`(보호자 본인 축이라 wardId 없음, `missedAlertEnabled` 선택·null이면 유지). 피보호자별 설정과 구분 — 저쪽은 "무엇을 보낼지", 이쪽은 "무엇을 받을지". 기본값 ON(기본 OFF면 아무도 안 켜서 기능이 죽는다).
- **킬 스위치가 2차와 독립**(`medication.reminder.missed-alert.enabled`) — 보호자 요약을 꺼도 피보호자 복용 알림은 계속 나간다(테스트로 고정).
- **테스트**: `./gradlew build` **393건 / 실패 0**(복약 64건 = 2차 50 + 신규 14).
- **V37 실적용 검증 완료**(gosky 스키마 복제본): 오류 0, 타입 일치, UNIQUE 2개·FK 3개 CASCADE 확인. 실데이터로 ① 같은 (보호자,피보호자,날짜) **중복 거부** ② 다른 날짜 허용 ③ 보호자 탈퇴 시 설정·발송기록 CASCADE 삭제 확인. 임시 DB 삭제, **운영 dev DB 무변경**(V36 그대로).
- **범위 밖(후속)**: 약 수정 API(OCR 검수 흐름의 전제) · 약봉투 OCR(업로드·OCR 엔진·파싱·비동기 인프라 전무 + 민감정보 이슈) · 순응도 통계 · 알림톡 템플릿 심사.
- 상세: `docs/(2026-08-05) feature-medication-missed-alert.md`

## [2026-08-05] 약 수정 API (PATCH)

- **발단**: 등록·삭제는 있었지만 수정이 없어 오타 하나에도 삭제 후 재등록해야 했고, 그러면 지난 복용 이력이 새 약과 끊어졌다. OCR 인식 결과를 고쳐 저장하는 흐름(다음 과제)의 전제이기도 하다.
- **`PATCH /api/guardian/medication/{medicationId}`** — 모든 필드 선택, `null`은 미변경(`CameraUpdateRequest` 관례). `wardId`·`createdBy`는 **불변**(옮기려면 삭제 후 재등록). **마이그레이션 없음** → V38 미사용이라 병렬 세션과 번호 충돌 없음.
- **규약 ①(시간대만 바꾸면 시각도 따라간다)**: `timeSlot`이 바뀌었는데 `doseTime`이 없으면 **새 시간대 기본 시각**으로 갱신한다 — 안 그러면 "저녁 08:00"이 남는다. 시각을 명시하면 그 값을 쓰고, **같은 시간대를 재전송하면 시각은 유지**한다(07:30으로 맞춘 아침 약이 08:00으로 되돌아가면 안 된다).
- **규약 ②(메모는 빈 문자열로 삭제)**: `null`이 미변경이라 `null`로는 못 지운다. 반대로 **이름은 공백만 보내면 기존값 유지** — 빈 이름으로 덮어쓰면 목록에서 무슨 약인지 알 수 없다.
- **핵심: 복용 시각이 바뀌면 당일 발송 기록(attempt 1·2)을 삭제**해 새 시각 기준으로 다시 알림이 나가게 한다. 없으면 "아침 08:00 → 저녁 20:00"으로 고쳤을 때 이미 08:00 발송 기록이 남아 **오늘 저녁에 안 울린다**. **복용 체크(`medication_intake`)는 건드리지 않아** 이미 드신 약은 여전히 조용하고 응답의 `taken`도 유지된다. 이름·메모만 바꾼 경우엔 기록을 건드리지 않는다(중복 알림 방지).
- **인가**: 수정도 **보호자 전용**이라 역할 분리(불변 규칙 ①) 유지 — 피보호자는 자기 약도 못 고친다. 비ACTIVE 403 + `[IDOR-ATTEMPT]`, 없는·삭제된 약 404(연결 확인 전 차단). 새 ErrorCode 불필요.
...
## [2026-08-07] Swagger 태그 체계 정리

- **발단**: Swagger UI를 열어도 "우리 API에 뭐가 있는지"가 한눈에 안 들어왔다. 원인 셋 — ① 명명 규칙이 **3가지 혼재**(`역할 - 도메인` / `역할`만 / `도메인`만) ② **`피보호자` 태그 하나에 연결·SOS 발생·SOS 설정 3개 도메인이 뭉침** ③ `SwaggerConfig.tags()`에 **9개만 등록**돼 카메라·SOS·복약·알림 설정 6개가 설명 없이 뒤에 붙음(= **최근 기능일수록 문서가 부실**).
- **특히 `사용자` 태그**: 주 사용자가 피보호자·보호자인데 제3의 역할처럼 읽혀 혼동을 만들었다. 실제 내용은 "내 계정"(프로필·비밀번호·탈퇴)이라 **`공통 - 내 계정`**으로 개명.
- **확정 규칙**: 태그 = **`<역할> - <도메인>`**, 역할은 **공통 / 피보호자 / 보호자 / 관리자 4가지뿐**. "누가 쓰는지 → 무슨 기능인지" 두 단계로 읽히게 한다. **역할만 적은 태그**(`보호자` — 그 역할 API 전부처럼 읽힘)나 **도메인만 적은 태그**(`인증` — 호출자가 사라짐)를 새로 만들지 말 것. 설명은 **`SwaggerConfig.tags()`에 일원화**(컨트롤러에 흩어두면 태그 하나에 컨트롤러 여럿일 때 충돌하고 전체 점검이 불가).
- **결과**: **15개 태그 / 22개 컨트롤러 / 78개 엔드포인트**를 빠짐없이 분류. 표시 순서도 공통 → 피보호자 → 보호자 → 관리자로 고정.
- **통합 2건**(승인): `알림`+`알림 설정` → **`공통 - 알림 설정`**(FCM 토큰=기기 단위, 채널 ON/OFF=계정 단위를 한 곳에서) / `WardSos`+`WardSosSetting` → **`피보호자 - SOS`**(발생과 동작 설정은 같은 화면 흐름).
- **인라인 description 5건을 SwaggerConfig로 이관** — `AuthController`에 있던 **공통 응답 포맷·토큰 사용법 안내는 `공통 - 인증` 설명에 그대로 보존**했다.
- **`operationOrderCustomizer`**에 빠져 있던 SOS·카메라·복약 경로 12개 추가(기존엔 인증·사용자·공지·문의까지만 있어 신규 기능 경로 순서가 뒤죽박죽이었다).
- ⚠️ **새 컨트롤러 추가 시 `SwaggerConfig.tags()` 등록 필수** — 이번 정리의 원인이 바로 이 누락이라, 코드에 경고 주석으로 남겼다.
- **검증**: `./gradlew build` 통과(407건/실패 0). 교차 검증 스크립트로 **설명 없는 태그 0 · 사용처 없는 태그 0 · 중복 0** 확인. Docker 미가용으로 앱 기동 `/v3/api-docs` 확인은 못 해 정적 교차 검증으로 대체.
- **FE 영향 없음** — URL·요청·응답·권한 전부 그대로이고 Swagger 그룹명·순서·설명만 바뀐다.
- 상세: `docs/(2026-08-07) refactor-swagger-tags.md`

## [2026-08-26] SOS 흐름 정비 - 처리 결과(ACK) 제거 · 119 정책 재정의 · 발생 경로 기록

- **발단**: 프론트에 SOS 화면이 붙지 않은 상태에서 SOS 비즈니스 로직을 확정했다. ① 119는 **키패드 화면만 띄우고 실제로 걸지 않는다**(학생 프로젝트) ② SOS 누르면 보호자 자동 알림(FCM 실패 시 SMS) ③ 보호자를 고르면 바로 전화 ④ 이력은 전부 남기고 보호자가 조회 ⑤ **처리 결과(ACK)는 불필요**.
- **착수 전 확인(PHASE 0)에서 드러난 것**: ②④는 **이미 완전히 구현돼 있었고**(백엔드 변경 0), ③의 전화 연결도 FE에 이미 있었다. 실제로 손볼 것은 **ACK 제거 · 119 문구 재정의 · 발생 경로 기록** 세 가지뿐이었다.
- **ACK 제거(V39, 비가역)**: 보호자 앱에 입력 화면이 붙은 적이 없어 `ack_status`가 **전건 NULL**이었다. 이 상태로 관리자 화면에 "미처리 SOS" 지표를 만들면 발생한 SOS 100%를 "보호자 전원 무응답"으로 표시하는 **거짓 경보**가 된다. `PATCH .../ack` · WS `sos-acknowledged` · 컬럼 4종 + FK · `SosAckStatus` 등을 제거했다. **배포 서버 두 곳 모두 ACK 데이터 0건**을 확인하고 DROP했다. 되살리려면 **보호자 화면 연동이 선행**되어야 한다.
- **발생 경로(`trigger_type`)**: `SOS_BUTTON` / `GUARDIAN_CALL`. 보호자 카드를 눌러 전화할 때 프론트가 `POST /api/ward/sos`를 함께 호출하면 그 전화도 이력에 남는다. ⚠️ **표시 전용** - 두 경로 모두 ACTIVE 보호자 전원에게 동일하게 알림이 나간다(전화받은 보호자 외 나머지도 알아야 하므로). 알림 분기 용도로 쓰지 말 것.
- **119 정책**: `SosAction` 값·스키마는 그대로 두고 의미만 재정의했다. `CALL_119`는 "전화를 건다"가 아니라 **"119 화면을 띄운다"**는 뜻이다. `tel:119`는 다이얼러를 열어 통화 버튼을 누르면 실제로 걸리므로 **자체 키패드 UI**로 그리기로 확정.
- **부수적으로 잡은 버그**: `GuardianSosService.resolveWardNames()`가 빈 결과로 `Map.of()`를 반환해, 탈퇴 피보호자의 **익명 이력만 조회되면 `get(null)`이 NPE(500)**를 던졌다. `Collections.emptyMap()`으로 교체. 리팩터 이전부터 있던 문제이며 새로 추가한 테스트가 잡았다.
- **파괴적 변경(FE)**: 이력 응답에서 ack 4필드 제거 + `triggerType` 추가 / `PATCH .../ack` 삭제 / WS `sos-acknowledged` 삭제. 노션 3개 페이지에 반영했다.
- **검증**: `./gradlew build` **399건 / 실패 0**. 마이그레이션은 dev DB에서 트랜잭션 실행 후 롤백으로 사전 검증(기존 1행이 `SOS_BUTTON`으로 정상 백필, 스키마 원상 확인).
- 상세: `docs/(2026-08-26) refactor-sos-flow.md`

## [2026-08-27] 미복용 요약 알림 시각 - 보호자 직접 선택

- **발단**: 3차(미복용 보호자 요약)의 발송 시각이 `application.yaml` 전역 값 하나(21:00)뿐이라 모든 보호자가 같은 시각에 받았다. 바꾸려면 서버 환경변수를 고쳐야 했다.
- **확정**: 보호자가 **분 단위로 자유롭게**(00:00~23:59) 지정한다. 지정 시각은 **발송 시각이자 집계 상한**이다 - 그 시각까지 복용 시각이 지난 약만 센다. 이르게 잡을수록 포함되는 약이 줄어드는 것은 의도된 동작이며, 알림 본문에 "**19:30까지 예정된** 복약 N건 중 M건"으로 시각을 밝혀 오해를 막는다.
- **거부안 2건**: ① "발송 시각만 선택, 집계는 항상 21:00" - 12:00에 "22:00 약 미체크"를 통보해 **매일 거짓 경보**가 된다(불변 규칙 ⑧이 막는 케이스). ② "이른 시각 + 21:00 하루 두 번" - 하루 1건 규칙(⑨) 위반, 알림 피로로 보호자가 앱 알림을 통째로 끄면 SOS·이상감지까지 죽는다.
- **집계 구조 재구성**: 보호자마다 상한이 달라 "약 → 피보호자 집계 → 보호자" 순서를 유지할 수 없다. **현재 시각까지의 약을 상위집합으로 한 번 읽고 보호자별 상한으로 다시 센다**(발송 창 안 보호자의 지정 시각은 언제나 현재 시각 이하라 상위집합이 성립). 미체크가 하나도 없는 피보호자를 먼저 거르는 기존 최적화는 그대로 남겼다.
- **매 분 전체 스캔 방지**: 설정 테이블의 `MIN`/`MAX(missed_alert_time)`과 전역 기본값으로 전체 발송 구간을 먼저 잡고, 그 밖이면 **복약 테이블을 조회하지 않고 종료**한다. 게이트가 없으면 스케줄러가 하루 종일 매 분 복약 전체를 훑는다.
- **V40**: `guardian_medication_setting.missed_alert_time TIME NULL`. **NULL = 미설정 → 전역 기본값**이라 백필 불필요. NOT NULL DEFAULT '21:00'으로 박으면 기존 보호자 전원이 고정돼 서버 기본값을 바꿔도 따라오지 않는다.
- **API 하위호환 유지**: `PUT /api/guardian/medication-alert-setting`의 두 필드 모두 선택(null=미변경)이라 기존 프론트의 `{missedAlertEnabled}` 요청이 그대로 동작한다. 기본값으로 되돌리려면 21:00을 직접 지정한다(null은 규약상 초기화 신호로 못 쓴다).
...
## [2026-08-31] 이상감지 판정 스키마 도입(V42) → 같은 날 전량 롤백(V43)

- **도입(PR #234)**: AI가 `danger=true`로 올린 감지가 실제 위험인지 오탐인지 서버는 알 수 없어(`confidence`는 "얼마나 불꽃처럼 보이는가"이지 "실제로 불이 났는가"가 아니다), 현장을 아는 보호자가 사후 판정하도록 스키마를 세웠다. `anomaly_incident`(판정 단위)·`anomaly_incident_feedback`(보호자 1인 1표)·`anomaly_event.incident_id` + 연속 감지 묶음 로직.
- **롤백(PR #235)**: 같은 날 회의에서 설계를 다시 확정하기로 해 되돌렸다. **V42 파일은 지우지 않고 V43으로 DROP** - 이미 적용된 서버가 있어 파일만 지우면 Flyway가 "적용됐지만 로컬에 없는 마이그레이션"으로 판단해 기동을 거부한다. 되돌리기도 앞으로 나아가는 마이그레이션으로 했다.
- 판정 API가 머지된 적이 없고 `anomaly_event`도 양쪽 서버 0건이라 데이터 손실 없음.

## [2026-09-01] 이상감지 판정 스키마 복원 (V44) + 미응답 재촉 설계 확정

- **발단**: 대시보드를 08-31 설계 그대로 가기로 확정돼, 롤백했던 PR ①을 되살렸다. 스키마·상황 묶음·알림 payload `incidentId`까지 `ff7f1ee` revert로 복원하고 **마이그레이션만 V44로 새로 썼다**(`V44__restore_anomaly_incident.sql`). V42·V43은 **두 서버 모두 적용 이력이 있어** 파일을 지우거나 고칠 수 없다 - 롤백 커밋은 "배포된 곳은 vkcs-linux 한 곳"이라고 적었지만 gosky도 V43까지 적용돼 있었다(양쪽 상태가 같아 V44 하나로 통일된다).
- **복원 안전성 확인**: `git diff 37cad0e HEAD`(anomaly·yaml·test 경로)가 **완전히 비어** 리포가 머지 이전과 바이트 단위로 동일했고, 롤백 이후 그 경로를 건드린 커밋도 없어 revert가 나중 변경을 덮어쓸 위험이 없었다. V44의 DDL은 주석을 뺀 본문이 V42와 완전히 동일하다.
- **재촉 설계 확정(구현은 PR ②)**: 판정은 관리자 운영 자료라 응답률이 곧 자료의 질이지만, 답할 때까지 계속 쏘면 보호자가 앱 알림을 통째로 꺼 **그 집의 SOS·화재 알림까지 죽는다**(복약 미복용 요약 불변 규칙 ⑨와 같은 판단). 그래서 **하이브리드**로 갔다 - 상황이 닫힌 뒤 1시간 경과 + PENDING이면 건별 FCM 1회, 이후는 하루 1회 미응답 요약. 마감 3일, 야간(22:00~08:00 KST) 억제, 채널은 FCM만, `SETTINGS_ONLY`에 보호자별 끄기 설정, 선점 후 발송 + `UNIQUE(incident_id, guardian_id, attempt)`, 킬 스위치 독립.
  - **다중 보호자**: 한 명이라도 응답해 PENDING을 벗어나면 **나머지 재촉만 중단**하고 응답 API는 계속 열어둔다. 엇갈림(`CONFLICTED`) 발견 경로는 살아 있고 알림량은 1/3이 된다. "전원 응답을 받아야 엇갈림을 안다"는 이유로 뒤집지 말 것.
  - **재촉 시작을 상황이 닫히기 전으로 당기지 말 것** - 대응 중인 사람에게 판정을 묻는 꼴이 된다.
- **대시보드 지표 보정**: 오탐률은 **응답률과 함께** 표시하기로 했다. 재촉해도 응답률은 100%가 되지 않아 오탐 건수만 띄우면 분모가 거짓이 된다. SOS 처리결과(ACK)가 전건 NULL인 채 "무응답 100%"로 읽힐 뻔한 함정과 같다.
- **PR 순서 조정**: 재촉은 **PR ②(응답 API)와 같은 PR**로 간다 - 누를 화면이 없는데 재촉만 보내면 받은 사람이 갈 곳이 없다. 마이그레이션은 ①=V44 / ②=V45 / ④=V46으로 밀렸다.
- **검증**: `./gradlew build` **426건 / 실패 0**(롤백 전 기준선과 일치, 415 → 426). V44는 vkcs-linux dev DB에서 트랜잭션 실행 후 롤백으로 확인(테이블 2·FK 4·CHECK 2·UNIQUE 1·인덱스 4 생성 → 롤백 후 원상).
- 상세: `docs/(2026-08-31) api-contract-anomaly-dashboard.md`(머리말·§4-3·§5-1·§8 갱신)

## [2026-09-01] 보호자 이상감지 이력·오탐 응답 + 미응답 재촉 (PR ②, V45)

- **범위**: 계약 §4-1·4-2(이력 조회·오탐 응답)에 §4-3(미응답 재촉)을 더했다. 재촉을 같은 PR에 넣은 이유는 응답 버튼이 없는데 재촉만 보내면 받은 사람이 갈 곳이 없기 때문이다. 판정 데이터가 안 쌓이면 SOS 처리결과(ACK)처럼 죽는다.
- **WIP 재사용**: 롤백 전 로컬에 남아 있던 `eec1e45`가 현재 dev 위에서 충돌·컴파일 오류 없이 붙었다(`f76beda` 기반 = V44가 복원한 상태와 동일). §4-1·4-2는 사실상 완성 상태였고 **미완성이던 건 테스트 0건**이었다. 실제 신규 작업은 재촉·테스트·Swagger 등록이다.
- **인가**: ACTIVE 연결이 유일한 열람 근거(`getActiveWardIds`·`isActiveConnection`, `getMyWards` 금지). 위반 403 `ANOMALY_NOT_AUTHORIZED` + `[IDOR-ATTEMPT]` WARN / 없는 상황 404 / 관리자 확정 409.
- **상태 재계산은 다수결이 아니다**: 전원 일치면 REAL·FALSE_ALARM, 갈리면 CONFLICTED. 표를 세어 한쪽으로 정하면 "보호자끼리 판단이 갈렸다"는 정보 자체가 사라지는데, 그게 관리자가 확인해야 할 신호다.
- **재촉(하이브리드)**: 상황이 닫힌 뒤 1시간 + PENDING이면 건별 FCM 1회 → 이후 하루 1회 요약(20:00 KST, 미복용 요약 21:00과 겹치지 않게). 마감 3일(상황 **시작** 기준), 야간 22:00~08:00 억제(버리지 않고 아침으로 미룸 - 화재 알림 본체는 밤에도 나간다), 스케줄러 5분.
  - **재촉하지 않는 다섯**: 이미 응답 / 상황이 PENDING을 벗어남 / 연결 해제 / 설정 OFF / 마감 초과. 특히 **한 명이 답하면 나머지 재촉만 중단하고 응답 API는 계속 열어둔다** - CONFLICTED 발견 경로는 살아 있고 알림량은 1/3이 된다.
  - **선점 후 발송** - 기록을 먼저 커밋하고 보낸다. 뒤집으면 5분마다 도는 스케줄러가 마감 내내 같은 재촉을 반복한다. 대가로 실패 회차는 유실되며, 하루 1회 요약이 두 번째 기회다.
- **V45는 테이블 2개로 나눴다**(설계 초안과 다름): `anomaly_review_reminder_log`(건별, UNIQUE (incident_id, guardian_id)) + `anomaly_review_summary_log`(요약, UNIQUE (guardian_id, summary_date)) + `guardian_anomaly_setting`. 요약은 축이 (보호자, 날짜)라 상황 단위 키에 담기지 않고, 그러고 나면 초안의 `attempt`는 항상 1인 죽은 컬럼이 되어 "여러 번 보낼 수 있다"는 잘못된 여지만 남는다.
- **공용 알림 인프라 변경**: "FCM만"이 기존 구조로 표현되지 않아(`SETTINGS_ONLY`는 사용자가 켠 모든 채널로 나간다) `NotificationType`에 **종류별 허용 채널**을 선언하고 디스패처가 *사용자 활성 채널 ∩ 허용 채널*로 발송하게 했다. 기본값은 전 채널이라 기존 종류는 무변경이고, **강제 채널(FCM)은 이 값으로 줄어들지 않는다**(줄이면 SOS·화재 푸시 보장에 구멍). 호출자 파라미터가 아니라 타입 선언이라 2026-07-27에 거부한 "디스패처 제외 채널 파라미터"와는 다르다.
- **문구는 단정하지 않는다**: "화재가 발생했습니다"가 아니라 "화재 감지가 있었습니다. 실제 상황이었는지 확인해 주세요". 실제 위험이었는지는 아직 아무도 모르고 그걸 묻는 알림이다(복약 "체크되지 않았습니다"와 같은 판단).
...
## [2026-09-02] FCM 토큰 누적 정리 (마이그레이션 없음)

- **발단**: "연결 수락 알림이 2번 온다"는 제보를 gosky 로그로 조사했다. 중복 토스트의 원인은 **프론트의 이중 렌더링**이었고(WebSocket + FCM을 각각 토스트로 그림 — FE 담당, 별건), 그 과정에서 일부 계정에 FCM 토큰이 2~3개 쌓여 있는 것이 드러났다. gosky 6명/12개, vkcs-linux 0개.
- **진짜 결함은 "누적"이 아니라 "판단 근거가 없다"였다**: `registerToken`이 같은 사용자의 재등록을 no-op으로 처리해 바뀌는 필드가 없었고, Hibernate가 UPDATE를 내지 않아 `updated_at`이 최초 등록 시각에 멈춰 있었다(12개 중 11개가 `updated_at = created_at`). 그 상태로 "60일 미갱신 삭제"를 켜면 **살아 있는지와 무관하게** 지운다 — 발송 로그는 `성공=3, 실패=0`으로 FCM이 전부 유효하다고 판정하고 있었다.
- **변경 3가지**: ① 재등록 시 `updated_at` 갱신(`@Modifying` UPDATE — 변경 감지에 맡기면 바꿀 필드가 없어 UPDATE가 안 나간다) ② 사용자당 상한 5개, 초과 시 마지막 사용이 오래된 것부터 삭제 ③ 60일 미갱신 토큰 정리(매일 04:00 KST, 발송이 몰리는 시간대를 피함).
- **상한 검사는 신규 등록 직후에만** 한다 — 재등록은 개수를 늘리지 않는다.
- **마이그레이션 불필요**: `created_at`·`updated_at`이 V8부터 있고 DB 트리거도 이미 있다.
- **배포 시 8개가 삭제된다**(60일 이상 미갱신). `GRD001`·`wDLVW1`은 토큰 0개가 되지만 전부 개발 테스트 계정이고, 프론트 `RoleRouteGuard`가 보호 라우트 진입 시 재등록하므로 다음 접속에 자동 복구된다. ⚠️ 열어둔 탭은 `sessionStorage`에 등록 기록이 남아 서버를 다시 부르지 않으므로 새로고침이 필요하다.
- **남겨둔 위험**: 토큰 0개인 동안 SOS는 SMS 폴백으로 살아남지만 **이상감지는 `FORCED_PUSH_PLUS_SETTINGS`라 SMS 폴백이 없어** 문자를 안 켠 사용자에게는 조용히 유실된다. 실사용자 전환 시 장기 미접속 피보호자에 대해 이 조합을 재검토할 것.
- **킬 스위치** `notification.fcm-token.cleanup-enabled`는 **정리만** 멈춘다(등록·발송은 그대로).
- **검증**: `./gradlew build` **488건 / 실패 0**(기존 480 + 신규 8).
- 상세: `docs/(2026-09-02) fix-fcm-token-accumulation.md`

## [2026-09-02] 관리자 이상감지 로그 + 판정 정정 (PR ④, V46)

- **범위**: 계약 §6의 `GET /api/admin/anomaly`(상태·피보호자 필터, 페이징, 보호자 응답 내역 포함) + `PATCH /api/admin/anomaly/{incidentId}/review`(정정). 이 PR로 **`resolvedBy`를 채우는 경로가 처음 생겼다** - 그전까지는 아무도 채울 수 없어 보호자 화면의 `resolvedByAdmin`이 항상 false였고 엇갈린 판정이 영원히 CONFLICTED로 남았다.
- 🔴 **V46이 필요한 이유가 예상과 달랐다**: 정정용 컬럼(`resolved_by`·`resolved_at`·`review_note`)은 **V44가 이미 만들어 뒀고** 엔티티 매핑까지 끝나 있었다. 관리자 확정이 보호자 응답으로 안 뒤집히는 것도 `applyReviewStatus()`에 이미 구현돼 있어 **테스트로 고정만** 하면 됐다.
  - 실제로 마이그레이션이 필요한 지점은 **감사 로그**였다. `admin_audit_log.action`에 CHECK 제약이 있어 enum에 값만 더하면 insert가 실패하고 **같은 트랜잭션의 정정까지 롤백돼 500**이 난다. V1의 CHECK가 V14의 `ANNOUNCEMENT_DRAFT_*`를 허용하지 않아 공지 임시저장이 통째로 죽었던 C-S3-1과 같은 함정이고, `AdminAuditActionCheckSyncTest`가 그 재발을 막는다(enum만 늘리면 테스트가 먼저 실패한다).
  - V46의 허용 목록은 V1이 아니라 **V27**(enum 전수와 맞춘 판)을 기준으로 삼았다. V1에 있던 `ANNOUNCEMENT_PUBLISH`는 enum에 없어 V27이 이미 뺐으므로 되살리지 않았다.
- **관리자는 1차 판정을 하지 않는다**: 여기 있는 것은 보호자 응답을 보고 뒤집는 2차 정정뿐이다. 관리자용 최초 판정 API를 추가하지 말 것.
- **정정은 상태만 바꾸고 보호자 응답 원본은 지우지 않는다** - 지우면 관리자가 무엇을 근거로 뒤집었는지 확인할 수 없어 정정 자체를 검증할 방법이 사라진다.
- **되돌리기는 막고, 재정정은 허용한다**: 지정 가능한 값은 REAL·FALSE_ALARM뿐이고 PENDING·CONFLICTED로 되돌리면 400(`ANOMALY_INVALID_REVIEW_STATUS`) - 확인을 마친 뒤 "아직 아무도 안 봤음"으로 되돌리면 그 상태의 뜻이 사라진다. 반면 이미 정정한 건의 재정정은 허용한다(관리자도 잘못 누를 수 있는데 막으면 되돌릴 방법이 없다). 매 정정이 감사 로그에 쌓여 추적된다.
- **감사 로그를 남긴다**: 개인 이력을 뒤집는 조작이라 반드시 기록한다. 집계 숫자만 보는 대시보드(PR ③)가 남기지 않는 것과 대비되며, 그때 한정해 둔 "개인 이력을 여는 관리자 API는 반드시 남긴다"는 예외가 여기 적용된다.
- **연결 여부로 좁히지 않는다**: 관리자는 전체를 봐야 엇갈린 판정을 찾을 수 있다. 그 대가가 감사 로그다.
- **정정 알림은 보내지 않는다**(판정은 이미 나간 알림을 되돌리지 않는다는 규칙 그대로).
...
## [2026-09-09] 관리자 회원 관리 (V47·V48)

- **범위**: `/api/admin/user` 5종 - 목록(검색·역할/계정/연결 상태 필터·페이징) · 탭 건수 · 상세 · 정보 수정 · 강제 탈퇴. 관리자 콘솔 회원관리 프로토타입에 대응한다.
- **백지 구현이 아니라 복원이었다**: 회원관리 API는 2026-05-15에 완성됐다가(PR #121·#122) 2026-06-11 "검증되지 않은 API 정리"(`a5b6328`)에서 통째로 제거됐고, `UserRepository:43`에 "구현 시 git 이력에서 복원할 것" 주석이 남아 있었다. **그대로 되살리면 안 되는 것이 PHASE 0에서 여섯 개** 나왔다.
- 🔴 **정지에 INACTIVE를 쓰면 계정이 삭제된다**: 옛 구현의 상태 토글은 `ACTIVE ↔ INACTIVE`였는데, 지금 INACTIVE는 탈퇴 전용이고 `WithdrawnUserPurgeScheduler`가 10분 지난 행을 **영구 삭제**한다. 2026-06-11 INACTIVE 불변식이 "그런 기능 도입 시 반드시 별도 상태값(예: RESTRICTED)을 추가할 것"이라고 예고해 둔 바로 그 경우라, `RESTRICTED`를 신설했다. 계정 상태는 **이용 중 / 이용 제한 / 삭제** 세 가지이고 "비활성화"는 이미 삭제가 있어 넣지 않았다.
- 🔴 **V47은 enum 추가만으로 끝나지 않는다**: `users.status`에 CHECK가 있고 **V4가 허용 목록을 두 값으로 좁혀** 놨다. enum만 늘리면 정지 UPDATE가 CHECK 위반(23514)으로 500이 난다 - `admin_audit_log.action`에서 두 번 터진 함정(C-S3-1, V46)과 같다. 재발 방지로 **`UserStatusCheckSyncTest`** 를 추가했다(`AdminAuditActionCheckSyncTest`와 같은 방식으로 enum 전수와 최신 CHECK를 대조).
- 🔴 **로그인 차단이 등호 비교라 정지가 뚫려 있었다**: `AuthService` 2곳·`KakaoAuthService` 1곳이 `status == INACTIVE`로 비교해 RESTRICTED가 그대로 로그인됐다. `!= ACTIVE`로 바꿔 새 상태값이 늘 때 조용히 뚫리는 형태를 없앴다. 응답은 기존 `INACTIVE_USER`("사용이 제한된 계정입니다") 재사용.
- **정지는 즉시 듣는다**: 상태만 바꾸면 이미 발급된 access token이 만료(30분)까지 살아 있어(`JwtAuthenticationFilter`는 요청마다 DB를 안 읽는다) `UserRestrictedEvent`로 탈퇴·비밀번호 변경과 같은 무효화 경로(refresh 삭제 + Redis 키)를 태운다.
- **역할 변경은 연결을 정리하고 상대에게 알린다**: 탈퇴 정리와 같은 규칙 - **ACTIVE는 `disconnect()`+알림, PENDING은 `cancel()`(무알림)**. 옛 구현은 ACTIVE도 `cancel()`로 끝냈는데 지금은 CANCELLED("수락 전 요청 취소")와 DISCONNECTED("연결 해제")로 뜻이 갈려 이력이 사실과 달라진다. PENDING 무알림은 2026-05-28 알림 비대칭 정책 그대로다. ADMIN 승격은 400.
- **강제 탈퇴는 일반 탈퇴와 같은 2단계 경로**: `userRepository.delete()` 직접 호출은 AFTER_COMMIT 리스너를 건너뛰어 상대 알림·FCM 정리·WITHDRAW 로그를 유실시키는데, 행 정리는 FK CASCADE가 해버려 **겉보기엔 성공한 것처럼 보인다**. 본인 확인만 뺀 `forceWithdraw()`를 추가했다. **감사 로그는 삭제보다 먼저** 남긴다(뒤에 남기면 기록 실패 시 계정만 사라지고 누가 지웠는지가 없다).
- **수정할 수 있는 것은 이름뿐**: 프로토타입엔 이메일·전화 입력창이 있었지만 둘 다 읽기 전용으로 정했다. 전화는 본인 변경 시 SMS 인증 nonce 소비가 필수라(H-5) 관리자 경로가 그 인증을 우회하고 잘못 넣으면 SOS·복약 문자가 남의 번호로 간다. 이메일은 본인조차 못 바꾸는 로그인 ID다. 이름 수정 때문에 `USER_NAME_CHANGE`가 생겨 **V48**(CHECK 재정의)이 딸려 왔다.
- **저장은 한 번, 감사 로그는 축별로**: 화면의 저장이 한 번이라 `PATCH` 하나로 받되 실제로 바뀐 축마다 따로 기록한다. 값이 같으면 아무것도 남기지 않는다(저장만 눌러도 이력이 쌓이면 진짜 변경이 묻힌다).
- **관리자 계정의 연결 상태는 NONE이 아니라 `null`**: 연결이 0건인 게 아니라 축 자체가 없는 계정이라, "미연결"로 표시하면 "연결이 끊긴 회원"으로 정확히 반대로 읽힌다("모르는 값을 0으로 채우지 않는다"와 같은 판단). 관리자는 목록·탭 건수엔 나오지만 수정·삭제는 403이다.
- **필터는 전용 enum, 수정 요청은 도메인 enum**: 필터에서 `Status`를 그대로 열면 `status=INACTIVE`가 빈 배열로 응답돼 "탈퇴 회원 0명"으로 오해된다(`WardListFilter`와 같은 판단). 반면 수정 요청은 `INACTIVE`를 400으로 분명히 거절해 "탈퇴는 상태가 아니라 삭제"를 전달한다.
- **관계(relation) 방향**: 1차 프로토타입의 양방향 라벨(`박민수 (부)` / `홍길동 (아들)`)은 **데이터로 만들 수 없었다** - `connections.relation`은 보호자를 가리키는 한 방향만 저장한다. 2차에서 "이 회원은 박민수님의 아들"로 방향이 맞춰졌고, 서버는 원본과 `counterpartRole`을 내리고 해석 규칙을 Swagger에 명시했다. 새 관계 13종은 모두 10자 이내라 **백엔드 변경 없음**.
- **범위 밖**: 강제 연결·해제 API(별도 PR) / 대시보드 집계(`Status.ACTIVE`만 세어 정지 계정이 총 회원 수에서 빠지지만, 계약이 따로 잡힌 화면이라 여기서 바꾸지 않았다) / 피보호자가 보내는 연결 요청(엔드포인트 자체가 없다).
- **검증**: `./gradlew build` **533건 / 실패 0**(기존 508 + 신규 25). V47·V48은 gosky dev DB에서 트랜잭션 실행 후 롤백으로 확인했고, 정지 UPDATE·새 감사 액션 insert가 통과하는 것과 **정의되지 않은 값(`DORMANT`)은 여전히 거부되는 것**까지 봤다.
- 상세: `docs/(2026-09-09) feature-admin-user-management.md`

## [2026-09-09] 이용 제한 계정 - 알림 차단 · 정지 사유 · 피보호자 정지 금지 (V49)

- **발단**: 회원관리(PR #243)로 계정 정지가 생겼는데 **정지된 계정에 알림이 계속 나갔다.** 디스패처가 사용자 상태를 보지 않았다. 앱은 못 여는데 SOS·화재·복약 알림은 도착하고 문자 요금까지 나간다.
- 🔴 **강제 채널(FCM)까지 막는다**: `WARD_SOS`·`ANOMALY_DETECTED`는 설정을 무시하는 필수 알림이라 "막아도 되나"가 쟁점이었는데, **막는 쪽이 맞다.** 정지의 용도를 "탈취 의심 시의 임시 조치"로 정했기 때문이다 - 그 계정을 누가 쥐고 있는지 모르는 상태에서 알림을 계속 보내면, 탈취된 보호자 계정에 **피보호자의 SOS 발생 시각·화재 감지가 실시간으로 통보된다.** "안전을 위해 알림을 이어간다"가 아니라 취약한 노인의 생활 패턴을 공격자에게 흘리는 것이 된다.
- **막는 기준은 수신자다**: 정지된 피보호자 집의 화재는 **그 보호자들에게 그대로 발송된다.** 계정 정지는 이용 제한이지 안전망 해제가 아니다.
- **받아들인 위험**: 보호자가 한 명뿐인데 그가 정지 상태면 그 피보호자의 SOS는 아무에게도 안 간다. `[NOTIFY-BLOCKED]` WARN으로 흔적만 남긴다. **"유일한 보호자 경고"는 검토 후 만들지 않기로 했다**(2026-09-09) - 정지 대상이 보안 이상·법적 문제 계정으로 한정돼 경고를 봐도 결정이 바뀌지 않는다. 미구현이 아니라 의도적 미채택이다.
- **차단 지점은 한 곳, 추가 쿼리 0**: `dispatch()`가 단일 진입점(호출부 12곳)이고 `NotificationRecipientResolver`가 이미 `findById()`로 User를 읽고 있어, `NotificationRecipient`에 상태를 실어 판정한다. ⚠️ **처음 구현에서 기존 최적화를 깼다가 되돌렸다** - `SETTINGS_ONLY`는 "활성 채널 0건이면 수신자 조회조차 안 한다"가 테스트로 고정돼 있었다. 지금은 채널 계산이 먼저고 그 다음 조회·차단이다.
- **상태 불명은 막지 않는다**: 사용자 행을 못 찾으면 `status`가 null인데 이건 "정지"가 아니라 "알 수 없음"이다. 기존 "부분 정보로도 발송" 동작 유지.
- 🔴 **정지 사유(V49)를 감사 로그에만 남기면 아무도 볼 수 없다**: 관리자 감사 로그 *조회* API가 2026-06-11에 제거돼 쓰기 경로만 남아 있어, DB를 직접 열지 않으면 "왜 잠겼는지"에 답할 수 없다. 정지가 "본인 확인까지의 임시 조치"인 이상 해제 판단을 하려면 사유가 화면에 보여야 해서 `users.status_reason` 컬럼을 두고 목록·상세 응답에 실었다. 해제하면 지운다(잠기지 않았는데 잠긴 이유가 남으면 안 된다). 변경 이력은 계속 `admin_audit_log`가 담당한다.
- **피보호자는 이용 제한할 수 없다(400)**: 정지하면 로그인이 막혀 **SOS를 보낼 수 없다.** 피보호자 계정은 편의 기능이 아니라 안전망 그 자체다. 탈취가 의심되면 정지 대신 **비밀번호 재설정**이 맞다 - `PasswordChangedEvent`가 토큰을 끊어 공격자는 튕겨나가고 본인은 계속 쓸 수 있다.
- **조합으로 검사한다**: 축별로 따로 검사하면 "보호자를 정지 → 역할을 피보호자로" 두 단계로 금지된 상태를 만들 수 있고, 순서를 정해 검사하면 "제한을 풀면서 역할을 바꾸는" 정상 요청이 잘못 거부된다. `validateResultingCombination()`이 **바뀐 뒤의 조합**을 한 번에 본다.
- **범위 밖**: "정지하면 이 피보호자의 유일한 보호자가 사라진다" 경고(관리자 API가 연결을 역으로 세야 함) / SMS 인증번호(디스패처 미경유, auth 건) / WebSocket(~~정지되면 로그인 자체가 안 돼 세션 없음~~ → 2026-09-10 정정: 새 로그인만 막히고 **이미 열린 세션은 유지**된다. 수용한 한계로 정책 문서에 기록).
- **검증**: `./gradlew build` **543건 / 실패 0**(기존 533 + 신규 10). V49는 gosky dev DB에서 트랜잭션 실행 후 롤백으로 확인했고, 컬럼 생성·기존 14행 무영향·사유 저장·**200자 초과 거부**·롤백 후 원상까지 봤다.
- 상세: `docs/(2026-09-09) fix-notification-restricted-user.md`

## [2026-09-10] 관리자 강제 연결·해제 (마이그레이션 없음)

- **범위**: `POST /api/admin/connection`(강제 연결) + `DELETE /api/admin/connection/{id}`(강제 해제). 회원관리 상세 모달의 "연결" 영역에서 호출한다. **연결 조회 API는 만들지 않았다** - 회원 상세가 이미 그 회원의 연결 전체를 준다.
- **왜 필요한가**: 고객센터 문의를 받아 관리자가 대신 처리하는 경로다. **시니어가 수락 버튼을 누르지 못해** 가족이 연결하지 못하는 경우가 실제로 있다.
- **마이그레이션 0건**: 감사 액션 `FORCE_CONNECT`·`FORCE_DISCONNECT`가 enum과 V48 CHECK에 **이미 있었다**. `NotificationType`은 DB 미저장이라 값 추가에도 CHECK 함정이 없다(오늘 두 번 걸린 그 함정이 여기엔 없다).
- 🔴 **기존 알림을 재사용하면 거짓말이 된다**: `ConnectionAcceptedEvent`는 "피보호자가 수락했습니다"(수락한 적 없음), `ConnectionDisconnectedEvent`는 "보호자가 해제했습니다"(관리자가 함)로 나간다. 복약 "체크되지 않았습니다"·이상감지 "화재 감지가 있었습니다"와 같은 판단을 적용해 **새 종류를 만들었다**.
  - `NotificationType.CONNECTION_FORCED` + `ConnectionForcedEvent` 신설 → 보호자 "관리자가 OOO님과의 연결을 완료했습니다", 피보호자 "관리자가 OOO님을 보호자로 연결했습니다"
  - **피보호자에게 "연결됨"을 알리는 경로가 원래 없었다** - 늘 수락하는 쪽이라 본인이 이미 알기 때문이다. 강제 연결은 모르는 채로 생기므로 새로 필요했다
  - 해제는 `DisconnectedBy.ADMIN` 값만 추가해 "관리자가 연결을 해제했습니다"로 분기
- **강제 해제는 양쪽 모두에게 알린다**: 당사자가 끊을 때는 "해제하지 않은 반대편"에게만 보내면 되지만, 관리자가 끊으면 **둘 다 자기가 끊지 않았다.** 한쪽만 알리면 나머지는 연결이 사라진 것을 모른다.
- **수락 대기 요청이 있으면 승격시킨다**: 같은 쌍의 PENDING이 있으면 새로 만들지 않고 `activate()`. 실제 시나리오가 정확히 이것(요청은 갔는데 수락을 못 누른 상태)이라 **관리자가 대신 수락해 주는** 셈이고, 새로 만들면 같은 쌍의 연결이 둘이 된다.
- **동의 없이 관계를 만드는 조작이라** 양쪽 알림 + 감사 로그를 필수로 붙였다. 일반 연결은 피보호자의 수락이 곧 동의인데 강제 연결에는 그게 없다.
- **거부 조건**: 역할 불일치 400 / **이용 제한·탈퇴 진행 계정 400**(로그인도 알림도 안 되는 계정이라 연결해도 동작하지 않는다) / 이미 ACTIVE면 409 / 해제 대상이 ACTIVE가 아니면 409. ⚠️ 중복 검사 기준이 옛 구현(`StatusNot(CANCELLED)`)과 다르다 - V23으로 REFUSED·DISCONNECTED가 생겨 "종료된 연결은 재요청 허용"이 정책이 됐다.
...
## [2026-09-10] 점검 이슈 반영 ① - 역할 변경 카메라·토큰 / INACTIVE 404 / 게이트 이중화 (마이그레이션 없음)

- **범위**: 같은 날 세로·횡단 점검의 Medium 6건 + 관리자 횡단 2건 + Low 4건. PR ①(이 항목)은 마이그레이션 없음, A-2(문의 답변 감사 로그, V50)는 PR ②로 분리.
- 🔴 **M-3 결정 - 역할 변경 시 카메라를 지운다**: WARD→GUARDIAN이 되면 카메라 API(WARD 전용)를 못 써 아무도 지울 수 없는 고아가 되고 AI 구독·본인 화재 알림은 계속됐다. "카메라가 있으면 400"도 검토했으나, 역할 변경이 필요한 실제 경우가 "가족이 실수로 피보호자로 가입"이라 초기화 후 재등록으로 확정. `CameraService.deleteAllByWard` + 감사 로그 "(연결 N건 해제, 카메라 M대 삭제)". AI 구독 해제는 기존 카메라 삭제와 같이 하지 않는다(unsubscribe 액션이 없고, 미등록 세션 신호는 스킵된다).
- **M-1 역할 변경도 토큰을 끊는다**: `JwtAuthenticationFilter`가 토큰의 role 클레임으로 권한을 만들어 옛 역할이 30분간 살아 있었다. `UserRoleChangedEvent` → `handleRoleChanged`(정지와 동일 경로).
- **M-2 INACTIVE는 상세·수정·삭제도 404**: 목록에서만 빼 두니 `status=ACTIVE`로 탈퇴 중인 계정이 되살아났다. `getUserOrThrow`에서 걸러 목록과 모집단을 맞췄다.
- **M-4·M-5 문서 정정**: 관리자 감사 로그는 "열람" 아닌 "변경" 조작만 기록(구현이 원래 그랬다). 정지 시 이미 열린 WS 세션은 끊지 못한다는 한계를 명시(핸드셰이크가 Principal을 안 세워 `SimpUserRegistry`로 못 찾는다).
- **M-6 게이트 테스트 3건**: AdminUser·AdminConnection·GuardianAnomaly 컨트롤러. GuardianAnomaly는 경로 규칙 없이 클래스 `@PreAuthorize`만이 게이트라 이 테스트가 유일한 회귀 방어.
- **A-1 기존 관리자 컨트롤러 3종(공지·초안·문의)에 클래스 `@PreAuthorize`** - 신규 4종과 기준 통일(동작 변화 없음).
...
- **해결**: 발송 직전 최근 **10분** 내 같은 피보호자 SOS 건수를 세어 2회 이상이면 문구를 바꾼다 - `"김순자님이 계속 도움을 요청하고 있습니다. (최근 10분 내 3번째)"`. 1회 문구는 **글자 그대로 유지**(기존 프론트·테스트 계약). `repeatCount`를 WS 페이로드·FCM data에 추가(기존 키 불변).
- **알림 건수는 늘지 않는다** - 쿨다운 30초·fail-open·429 미도입·강제 FCM·문자 폴백·알림톡 미매핑 모두 그대로다. 바뀐 것은 문구뿐이다.
- **집계는 쿨다운 통과 후에만** - 순서를 뒤집으면 막힐 연타까지 매번 DB를 친다. 쿼리는 쿨다운 주기당 한 번이다.
- **집계 실패는 알림을 막지 않는다**(예외 삼키고 1회로 보아 기본 문구, WARN). 쿨다운 fail-open과 같은 원칙 - 집계는 문구를 친절하게 만드는 부가 정보이지 긴급 알림의 전제가 아니다.
- **문구는 단정하지 않는다** - 횟수는 확인된 사실이라 쓰되 위급도·원인은 말하지 않는다(복약 "체크되지 않았습니다"·이상감지 "화재 감지가 있었습니다"와 같은 판단).
- **집계 창 10분 ≠ 쿨다운 30초**: 쿨다운은 "얼마나 자주 보낼지", 창은 "얼마나 거슬러 한 상황으로 볼지". 설정 키로 빼지 않았다(SOS엔 설정 클래스가 없고 쿨다운도 상수 - 필요해지면 둘을 함께 승격).
...
## [2026-09-21] 이상감지 판정 E-2·E-3 보류 결정 (코드 미수정)

- #252 영향 범위 점검의 E-2(동시 응답 시 판정 덮어쓰기)·E-3(연결 해제·탈퇴 보호자의 표)을 **보류·수용**했다. 둘 다 다수결 전환 이전부터 있던 드문 경우이고, 운영 데이터 0건이며, E-2는 다음 응답에서 스스로 바로잡힌다.
...
## [2026-09-22] PR #254 머지·CD 확인 - 통합 테스트 캐시 재사용은 그대로 두기로
