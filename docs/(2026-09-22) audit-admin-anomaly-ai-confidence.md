...
## 조치 결과 (2026-09-22, 사용자 "전부 반영")

| # | 결과 |
|---|---|
| M-1 | ✅ Notion "관리자 API - 이상감지 로그" 응답률 문구 M = `total - pending` |
| L-1 | ✅ `/summary` 역할별 테스트 3건 추가 |
| L-2 | ✅ `aiConfidence.real`·`falseAlarm` 제거(FE 연동 전) |
| L-3 | ✅ 파서에서 0~1로 보정 + WARN(수용 → 반영으로 변경, 신호는 버리지 않음) |
| L-4 | ✅ Swagger·DTO·Notion에 최고값 평균의 치우침 명시 |
