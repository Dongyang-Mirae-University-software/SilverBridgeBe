...
보호자가 한 명뿐인 피보호자에게서 SOS가 났는데 **그 보호자가 정지 상태면 아무에게도 가지 않는다.** 발송을 건너뛸 때마다 `[NOTIFY-BLOCKED]` WARN을 남겨 흔적은 만든다. 이 위험을 줄이는 "유일한 보호자 경고"는 검토 후 **만들지 않기로 했다**(§6-1).
...
- **"이 보호자를 정지하면 김영희의 유일한 보호자가 사라집니다" 경고** - **만들지 않기로 했다**(2026-09-09 결정, §6-1)
- **SMS 인증번호** - 디스패처를 거치지 않는 별개 흐름이라 정지 계정도 비밀번호 재설정 문자를 받을 수 있다. auth 도메인 건이다
- **WebSocket** - 추상화 밖이지만 정지되면 로그인 자체가 안 돼 세션이 없다. 손댈 것 없음

### 6-1. "유일한 보호자 경고"를 만들지 않기로 한 이유 (2026-09-09 결정)

관리자가 보호자를 정지·삭제·역할 변경할 때 "이 사람이 김영희의 유일한 보호자입니다"를 띄우자는 안을 검토했고, **채택하지 않았다.**

**정지 대상을 보안 이상·법적 문제 계정으로 한정했기 때문이다.** 그런 계정은 유일한 보호자여도 막아야 하므로 경고를 봐도 관리자의 결정이 달라지지 않는다. 결정을 바꾸지 못하는 정보는 화면만 복잡하게 만든다. 정지 계정이 0건인 현재로서는 쓰이지 않는 코드가 하나 느는 것이기도 하다.

**다만 위험이 사라진 것은 아니다.** 정지를 실제로 쓰기 시작해 이 경우가 생기면 그때 다시 판단한다. 이 항목은 **미구현이 아니라 의도적으로 만들지 않은 것**이므로 "빠졌다"고 보고 다시 만들지 말 것.

설계 과정에서 확인한 두 가지는 기록해 둔다. 나중에 만들게 되면 반드시 반영해야 한다.

1. **"다른 보호자가 있다"가 안심의 근거가 못 된다** - 다른 보호자가 있어도 **그 사람도 정지 상태**면 실제로는 아무도 못 받는다. 연결 상태만 보면 거짓 안심을 준다. 상대 보호자의 계정 상태까지 봐야 한다.
2. **정지가 만드는 사각지대는 대시보드에 안 잡힌다** - 강제 탈퇴·역할 변경은 연결이 사라져 `wardsWithoutGuardian`에 잡히지만, **정지는 연결이 그대로 남아** 지표상 멀쩡해 보인다. 셋 중 가장 위험한데 가장 안 보인다.
