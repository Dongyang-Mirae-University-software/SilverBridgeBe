...
발견 28건 중 24건 적용, 2건 엔지니어링 판단 제외, 1건(M-1) 프론트 처리로 방향 확정, 1건(키 회전) 운영 보류. 코드 조치는 모두 결함 단위 PR로 분리해 dev 머지 완료.
...
- M-1 카카오 OAuth state 검증 → 모델 A(프론트 검증)로 결정 (2026-05-18). 프론트가 state 생성·sessionStorage 보관·콜백 자체 대조를 수행하고, 백엔드에는 기존대로 `code`만 전달. 백엔드 코드 변경 없음. 프론트는 카카오 로그인 신규 구현 예정(현재 미구현)이라 처음부터 state 검증을 포함해 구현. 상세 명세는 프론트에 별도 전달 완료. 프론트 배포 확인 시 "해소 완료"로 갱신.
...
확인 결과: 프론트는 카카오 로그인을 아직 구현하지 않았음. 모델 A(프론트 검증)로 결정.
- 프론트가 카카오 로그인 신규 구현 시 state 생성·sessionStorage 보관·콜백 자체 대조를 포함해 구현.
- 검증 통과 후 백엔드(`POST /api/auth/signin/kakao`)에는 기존대로 `{ code }`만 전달. state는 백엔드로 보내지 않음.
- 백엔드 코드·API 계약 변경 없음 (KakaoLoginRequest 그대로). 신규 가입 흐름의 verificationNonce는 H-5(PR #131)로 이미 반영됨.
- 프론트에 신규 구현 명세 전달 완료. 프론트 배포 확인 시 본 항목을 "해소 완료"로 갱신 예정.

---

## 미해결 TODO (다음 사이클 이월)

- M-1 카카오 OAuth state 검증 — 모델 A로 결정됨(백엔드 작업 없음). 프론트 카카오 로그인 신규 구현·배포 확인 후 "해소 완료" 기록만 남음
