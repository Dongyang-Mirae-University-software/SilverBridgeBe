...
## 10. 확정 사항 (2026-07-09 결정, 1차 구현 반영)

| # | 항목 | 결정 | 비고 |
|---|---|---|---|
| 1 | 한 기기 다중 방 | **기기 1대 = 방 1개** | 추가 구현 없음 |
| 2 | 등록 주체 | **피보호자 본인만** | 보호자 대행 필요 시 `POST /api/guardian/cameras` 추가(ACTIVE 연결 검증) |
| 3 | 카메라 개수 상한 | **미적용** | 어뷰징 관측 시 도입 |
| 4 | FPS | **5fps 고정, 백엔드 config 소유** | `camera.recommended-fps`(env `CAMERA_RECOMMENDED_FPS`) → 등록 응답 `recommendedFps`로 내려줌. FE 매직상수 방지 |
| 5 | 방 이름 중복 | **허용**(라벨 유니크 미검사) | 기기 교체·재등록 유연성 |
| 6 | Phase C(스코프 스트림 토큰) | **다음 단계로 분리** | 1차는 백엔드 allowlist까지 |

### FPS = 5 근거
화재/연기는 느리게 변하는 대상이고 AI 서버가 `STREAM_SAMPLE_EVERY_N_FRAMES=5`로 5프레임마다 추론한다.
5fps 입력이면 추론이 초당 ~1회로 수초 내 검출이 보장되며, 그 이상은 대역폭·GPU만 소모하고 검출 이점이 미미하다.
(배포 실측 세션의 실제 FPS도 1.82였고, FE 업로드는 500ms 스로틀로 최대 2fps.)

### 1차 구현 범위에서 제외한 것
- `assertGuardianCanView(guardianId, sessionId)` **개별 인가 헬퍼**: 이를 호출하는 엔드포인트가 아직 없어 dead code가 되므로 제외.
  Phase C(스코프 스트림 토큰)에서 해당 엔드포인트와 함께 도입한다. 동일 이유로 `CAMERA_ACCESS_DENIED` 에러코드도 미추가
  (현재 필요한 것은 `CAMERA_NOT_FOUND` 404 위장 하나).
- `POST /api/ward/camera`는 신규/기존 모두 **200**으로 응답한다(설계 초안의 201/200 구분 폐기).
  코드베이스 관례(`GuardianInquiryController.create`도 200)를 따르며, FE는 응답의 `deviceId`를 저장하면 되므로 구분이 불필요.
