...
## 4. PR ② 보호자 - 이상감지 이력 조회 + 오탐 응답 <sub>(구현 완료 2026-09-01)</sub>
...
**구현하며 확정된 것 (2026-09-01, PR ②)**

- **테이블은 둘이다** - `anomaly_review_reminder_log`(건별, `UNIQUE (incident_id, guardian_id)`)와
  `anomaly_review_summary_log`(요약, `UNIQUE (guardian_id, summary_date)`). 축이 달라 나눴다.
  설계 초안의 `UNIQUE (incident_id, guardian_id, attempt)`는 **쓰지 않는다** - 건별은 상황당 보호자당
  정확히 1회라 `attempt`가 항상 1인 죽은 컬럼이 되고, "여러 번 보낼 수 있다"는 잘못된 여지를 남긴다.
- **요약에는 이미 건별 재촉을 보낸 상황만 담는다.** 이 조건이 없으면 1차가 아직 안 나간 상황이 요약에
  먼저 실려, 같은 상황으로 요약과 건별 재촉이 연달아 도착한다.
- **설정 API**: `GET·PUT /api/guardian/anomaly/reminder-setting`. 행이 없으면 기본 ON,
  `null`은 "변경하지 않음"(설정 API 공통 규약).
- **"FCM만"을 구조로 보장했다**: `NotificationType`에 **종류별 허용 채널**을 선언하고, 디스패처가
  *사용자가 켠 채널 ∩ 그 종류의 허용 채널*로 설정 기반 발송을 한다. 기본값은 전 채널이라 기존 종류의
  동작은 그대로다. **강제 채널(FCM)은 이 값으로 줄어들지 않는다** - 줄일 수 있게 만들면 SOS·화재의
  푸시 보장에 구멍이 생긴다. 호출자가 넘기는 파라미터가 아니라 타입에 박힌 선언이라는 점이 중요하다
  (디스패처 "제외 채널" 파라미터 거부안, 2026-07-27).
- **스케줄러 주기 5분**(`anomaly.review-reminder.*`). 기준이 "닫히고 1시간 뒤"라 분 단위 정확도가 필요 없다.
...
| ② | 보호자 이력 조회 + 오탐 응답 **+ 미응답 재촉(§4-3)** ✅ | **V45**(재촉 로그 2종·보호자 설정) | 화면 신규 |
