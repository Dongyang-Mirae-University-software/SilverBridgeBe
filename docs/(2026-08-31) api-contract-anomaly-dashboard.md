...
> 상태: **보류(2026-08-31)** - 회의에서 설계를 다시 확정하기로 해 구현 착수분(PR ①, V42)을 되돌렸다.
> 이 문서는 **회의 입력 자료**로 남긴다. 확정 후 다시 시작할 때는 여기서 바뀐 부분만 갱신해 쓰고,
> 마이그레이션은 V42를 되살리지 말고 **V44 이후로 새로 작성**할 것(V42 추가 → V43 되돌리기가 이미 기록돼 있다).
